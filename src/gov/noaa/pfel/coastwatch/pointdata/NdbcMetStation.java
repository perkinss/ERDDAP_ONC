...
public class NdbcMetStation  {
     ...
     */
    public static String firstNearRealTimeData = "2014-10-01T00:00:00";
...
<img src="/icons/text.gif" alt="[TXT]" /> <a href="42OTP.txt">42OTP.txt</a>               28-Aug-2005 14:23   24K  Standard Meteorological Data
    ...
    public static void test46088(Table table) throws Exception {
        ...
        //2014 10 01 01 20 270  9.0 11.0   0.8     4   3.4 261 1018.8  13.6  10.5   9.8   MM   MM    MM
        //2014 10 01 00 50 270  9.0 11.0   0.6     4   3.4 261 1018.7  13.7  10.5   9.5   MM +0.0    MM
        seconds = Calendar2.isoStringToEpochSeconds("2014-10-01T01"); //50 min rounds to next hour; usually test 01T01
        row = table.getColumn(timeIndex).indexOf("" + seconds, 0);
        Test.ensureEqual(table.getStringData(idIndex, row), "46088", "");
        Test.ensureEqual(table.getFloatData(latIndex, row), 48.333f, "");
        Test.ensureEqual(table.getFloatData(lonIndex, row), -123.167f, "");
        ...
        Test.ensureEqual(table.getFloatData(wvhtIndex, row), .8f, ""); 
        Test.ensureEqual(table.getFloatData(dpdIndex, row), 4f, ""); 
        Test.ensureEqual(table.getFloatData(apdIndex, row), 3.4f, "");
        Test.ensureEqual(table.getFloatData(mwdIndex, row), 261, "");  //or getIntData
        Test.ensureEqual(table.getFloatData(aprsIndex, row), 1018.8f, "");
        Test.ensureEqual(table.getFloatData(atmpIndex, row), 13.6f, "");
    ...
    public static void test46088AddLastNDays(Table table) throws Exception {
        ...
        //#YY  MM DD hh mm WDIR WSPD GST  WVHT   DPD   APD MWD   PRES  ATMP  WTMP  DEWP  VIS PTDY  TIDE
        //#yr  mo dy hr mn degT m/s  m/s     m   sec   sec degT   hPa  degC  degC  degC   mi  hPa    ft
        //2014 10 27 23 20 140  8.0 10.0   0.6     4   3.3 116 1015.9  12.3  12.0   8.6   MM   MM    MM
        //2014 10 27 22 50 140  7.0  9.0   0.6     4   3.4 116 1016.2  12.3  12.0   8.6   MM -1.9    MM
        double seconds = Calendar2.isoStringToEpochSeconds("2014-10-27T23"); //rounded
        int row = table.getColumn(timeIndex).indexOf("" + seconds, 0);
        ...
        Test.ensureEqual(table.getFloatData(dpdIndex, row), 4f, "");
        Test.ensureEqual(table.getFloatData(apdIndex, row), 3.4f, "");
        Test.ensureEqual(table.getFloatData(mwdIndex, row), 116f, "");
        Test.ensureEqual(table.getFloatData(aprsIndex, row), 1016.2f, "");
        Test.ensureEqual(table.getFloatData(atmpIndex, row), 12.3f, "");
    ...
    public static void main(String args[]) throws Exception {
        ...
        //  Use DOS window "dir" or Linux ls instead of the GUI.
        //downloadNewHistoricalTxtFiles(ndbcHistoricalTxtDir); //time varies, last done 2014-10-27

        // 3) *** get latest 45 day files
        //DON'T download45DayTextFiles after 45 days after last historicalTxt date.
        //download45DayTxtFiles(ndbc45DayTxtDir);  //15-30 minutes, last done 2014-10-27
        ...
        //    ndbcNcDir, ignoreStationsBefore, testMode); //M4700 ~1 hr, was ~3 hrs  //last done 2014-10-27
...
rename ndbcMett ndbcMetR20140726 ndbcMett
rm ndbcMett.tgz
        */
