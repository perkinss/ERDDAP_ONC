...
public class EDDTableFromCassandra extends EDDTable{ 
    ...
    protected HashSet constrainableSourceNames;
    ...
    public static EDDTableFromCassandra fromXml(SimpleXMLReader xmlReader) throws Throwable {
        ...
        String tConstrainableSourceNames = null;
        ...
        while (true) {
            ...
            else if (localTags.equals( "<constrainableSourceNames>")) {}
            else if (localTags.equals("</constrainableSourceNames>")) tConstrainableSourceNames = content; 
        ...
        return new EDDTableFromCassandra(tDatasetID, tAccessibleTo, 
                ...
                tPartitionKeySourceNames, tConstrainableSourceNames, 
    ...
    public EDDTableFromCassandra(String tDatasetID, String tAccessibleTo, 
        ...
        String tPartitionKeySourceNames, String tConstrainableSourceNames,
        ...
        constrainableSourceNames = new HashSet();
        if (String2.isSomething(tConstrainableSourceNames)) {
            String sar[] = String2.split(tConstrainableSourceNames, ',');  //they are trimmed
            for (String s: sar) 
                constrainableSourceNames.add(String2.canonical(s));
        }
    ...
    public static Session getSession(String url, 
        ...
        opt = "ssl";
        if (verbose) String2.log("  " + opt + " default=false"); //NONE
        ts = connectionProperties.get(opt);
    ...
    public void getDataForDapQuery(String loggedInAs, String requestUrl, 
        ...
        for (int cv = 0; cv < nCon; cv++) {
            ...
            //for WHERE below, just keep constraints applicable to constrainableSourceNames
            conKeep.set(cv, 
                constrainableSourceNames.contains(cVar) &&
                !cOp.equals("!=") && 
        ...
        //add constraints on constrainableSourceNames
        ...
        for (int pkdRow = 0; pkdRow < pkdTableNRows; pkdRow++) { //chunks will be in sorted order, yea!
            ...
            //get the data
            getDataForCassandraQuery(loggedInAs, requestUrl, userDapQuery,
    ...
    /** 
     * This executes the query statement and writes data to the tablewriter. 
     ...
     */
    public void getDataForCassandraQuery(
        ...
        for (int rv = 0; rv < nRv; rv++) {
            ...
            rvToRsCol[rv] = columnDef.getIndexOf(rvToResultsEDV[rv].sourceName()); 
        ...
        while ((row = rs.one()) != null) {
            ...
            for (int rv = 0; rv < nRv; rv++) {
                ...
                Class tClass = edv.sourceDataTypeClass();
                if (debugMode) String2.log(rv + " " + row.getString(rsCol));
    ...
    public static String generateDatasetsXml(
        ...
        //constrainable (could be a Set)
        StringArray constrainableSA = new StringArray(); //souceNames
        List<ColumnMetadata> cc = tm.getClusteringColumns();
        for (int i = 0; i < cc.size(); i++) 
            constrainableSA.add(cc.get(i).getName());
...
//are static variables constrainable?
                if (constrainableSA.indexOf(sourceName) < 0)
                    constrainableSA.add(sourceName);
            }
            
            if (cm.getIndex() != null) {
                //indexed columns are constrainable?
                if (constrainableSA.indexOf(sourceName) < 0)
                    constrainableSA.add(sourceName);
            }
            ...
            Attributes addAtts = makeReadyToUseAddVariableAttributesForDatasetsXml(
                sourceAtts, sourceName, true, true); //sourceAtts, sourceName, addColorBarMinMax, tryToFindLLAT
            ...
            String destName = 
                ...
                sourceName;
            if      (destName.equals(EDV.LON_NAME))   addAtts.add("units", EDV.LON_UNITS);
            else if (destName.equals(EDV.LAT_NAME))   addAtts.add("units", EDV.LAT_UNITS);
            else if (destName.equals(EDV.ALT_NAME))   addAtts.add("units", EDV.ALT_UNITS);
            else if (destName.equals(EDV.DEPTH_NAME)) addAtts.add("units", EDV.DEPTH_UNITS);
        ...
        sb.append(
            ...
            "    <constrainableSourceNames>" + constrainableSA.toString() + "</constrainableSourceNames>\n" + 
    ...
    public static void testGenerateDatasetsXml() throws Throwable {
        ...
        Attributes addGlobalAtts = new Attributes();
        //addGlobalAtts.
...
"    <constrainableSourceNames>sampletime</constrainableSourceNames>\n" +
"    <maxRequestFraction>1</maxRequestFraction>\n" + 
"    <columnNameQuotes></columnNameQuotes>\n" +
